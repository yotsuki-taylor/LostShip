...
| Icon | URL или путь к спрайту врага (например, `kraken.png` или `images/kraken.png`). **Имя файла лучше писать строчными буквами** — на Linux путь к картинке чувствителен к регистру (`Kraken.png` и `kraken.png` — разные файлы). В интерфейсе локальные пути приводятся к нижнему регистру. |
...
Каждая строка — отдельный враг (например, «Планарные пираты», «Кракен»). Ивенты боя (`EventStart`, `EventTurn*`, `EndFightEvent`) задаются **уникальными id** для каждого врага (см. локальный пример: `AirshipFight_*` и `KrakenFight_*` в `events.json`).
